...
public class BST<T extends Comparable<T>> extends BT<T> {
 ...
 public void insert(T newdata) {
  ...
  if (root == null) {
   ...
   // insert your code
   while(current!=null) { //current what we focus on think as it is a root
    if(newdata.compareTo(current.element)<=0) {
     parent=current; 
     current=current.left; //position is left
    }
    else {
     parent=current;
     current=current.right; //position is right
    }
   }
   BTNode<T> newnode=new BTNode<T>(newdata);
   if(newdata.compareTo(parent.element)<=0) {
    parent.left=newnode; //compare
   }
   else {
 ...
 public T delete(T item) {
  ...
  // Case 2.2: If its child node is on the left
   ...
   if (maxleft.left == null && maxleft.right == null) { // Case 3.1 if maxleft is a leave, then .. //check the leave
    //insert your code
    maxleftParent.right=null; //the right one gone
   }
   else if (maxleft.left != null) { // Case 3.2 if maxleft has a left child, then .
    //insert your code
    maxleftParent.right=current.left;
   }
 ...
 public boolean search(T data) {
  ...
  while(current!=null) {
   ...
   else {//compare and data=current element
    return true;
   }
  }
  return false;
